...
dat = mutate(dat, season = 1)
dat[which(dat$Month==1|2|3),]$season="winter"
dat[which(dat$Month==4|5|6),]$season="spring"
dat[which(dat$Month==7|8|9),]$season="summer"
dat[which(dat$Month==10|11|12),]$season="fall"
##
```
...
##obtain correlation coefficients of numeric covariates
select(dat, c(Year, Month, DayofMonth, DayOfWeek, Distance, TaxiIn, TaxiOut, TEMP.x, DEWP.x, SLP.x, VISIB.x,
              WDSP.x, MXSPD.x, MAX.x, MIN.x, PRCP.x, SNDP.x, TEMP.y, DEWP.y, SLP.y, VISIB.y,
              WDSP.y, MXSPD.y, MAX.y, MIN.y, PRCP.y, SNDP.y)) %>% cor()
##There is no highly correlations between pairwise covariates

##Normality: QQplot of responde variables: ArrDelay, DepDelay
pdf("~/final/ArrDelayplot.pdf")
qqnorm(dat$ArrDelay, pch = 1, frame = FALSE)
qqline(dat$ArrDelay, col = "steelblue", lwd = 2)
dev.off()
pdf("~/final/DepDelayplot.pdf")
qqnorm(test$DepDelay, pch = 1, frame = FALSE)
qqline(test$DepDelay, col = "steelblue", lwd = 2)
dev.off()
##
```
