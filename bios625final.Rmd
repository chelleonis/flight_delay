...
set.seed(1234)
dat1 = dat[complete.cases(dat$DepDelay),]
flds = createFolds(dat1$DepDelay, k = 10, list = TRUE, returnTrain = FALSE)
n=length(flds)
...
sum_rresidual=0
SSY=0
Ybar=mean(dat1$DepDelay)
n1=0
##form design matrix for the model
b = cbind(rep(1,nrow(dat1)),factorize(dat1$Year), dat1$Month, dat1$DayofMonth, factorize(dat1$DayOfWeek), dat1$Distance, dat1$TaxiIn, 
          dat1$TaxiOut, dat1$TEMP.x,dat1$DEWP.x, dat1$SLP.x, dat1$VISIB.x, dat1$WDSP.x, dat1$MXSPD.x, dat1$PRCP.x, dat1$SNDP.x, dat1$TEMP.y, 
          dat1$DEWP.y,dat1$SLP.y, dat1$VISIB.y, dat1$WDSP.y, dat1$MXSPD.y, dat1$PRCP.y, dat1$SNDP.y, factorize(dat1$UniqueCarrier), 
          factorize(dat1$Origin),factorize(dat1$Dest),factorize(dat1$Fog.x), factorize(dat1$Rain.x), factorize(dat1$Snow.x), 
          factorize(dat1$Hail.x),factorize(dat1$Thunder.x), factorize(dat1$Tornado.x), factorize(dat1$Fog.y), factorize(dat1$Rain.y), 
          factorize(dat1$Snow.y),factorize(dat1$Hail.y), factorize(dat1$Thunder.y), factorize(dat1$Tornado.y),factorize(dat1$season))
...
for (i in 1:n){
  a = biglm.big.matrix (DepDelay ~ Year + Month + DayofMonth + DayOfWeek + Distance + TaxiIn + TaxiOut + TEMP.x +
                    ...
                    Hail.y + Thunder.y + Tornado.y + season, data = dat1[-flds[[i]],] )
  x=b[flds[[i]],]
  c=x%*%summary(a)[[2]][,1]
  c=dat1[flds[[i]],]$DepDelay-c
  SSY=SSY+sum((dat1[flds[[i]],]$DepDelay-Ybar)^2)
  n1=n1+length(c)
...
Ybar=mean(dat1$DepDelay)
n1=0
b=cbind(rep(1,nrow(dat1)),factorize(dat1$Year), dat1$Month, dat1$DayofMonth, factorize(dat1$DayOfWeek), dat1$Distance, dat1$TEMP.x,
        dat1$DEWP.x, dat1$SLP.x, dat1$VISIB.x, dat1$WDSP.x, dat1$MXSPD.x, dat1$PRCP.x, dat1$SNDP.x, factorize(dat1$UniqueCarrier),
        factorize(dat1$Origin),factorize(dat1$Dest),factorize(dat1$Fog.x), factorize(dat1$Rain.x), factorize(dat1$Snow.x), 
        factorize(dat1$Hail.x),factorize(dat1$Thunder.x), factorize(dat1$Tornado.x), factorize(dat1$season))
for (i in 1:n){
a = biglm.big.matrix (DepDelay ~ Year + Month + DayofMonth + DayOfWeek + Distance + TEMP.x + DEWP.x + SLP.x + VISIB.x + WDSP.x + MXSPD.x + PRCP.x + SNDP.x + UniqueCarrier + Origin + Dest + Fog.x + Rain.x + Snow.x + Hail.x + Thunder.x + Tornado.x + season, data = dat1[-flds[[i]],] )
x=b[flds[[i]],]
c=x%*%summary(a)[[2]][,1]
c=dat1[flds[[i]],]$DepDelay-c
SSY=SSY+sum((dat1[flds[[i]],]$DepDelay-Ybar)^2)
...
###PRESS for ArrDelay
set.seed(1234)
dat1 = dat[complete.cases(dat$ArrDelay),]
...
SSY=0
b=cbind(rep(1,nrow(dat1)),factorize(dat1$Year), dat1$Month, dat1$DayofMonth, factorize(dat1$DayOfWeek), dat1$Distance, dat1$TEMP.y, dat1$DEWP.y,
        dat1$SLP.y, dat1$VISIB.y, dat1$WDSP.y, dat1$MXSPD.y, dat1$PRCP.y, dat1$SNDP.y, factorize(dat1$UniqueCarrier), factorize(dat1$Origin),
        factorize(dat1$Dest), factorize(dat1$Fog.y), factorize(dat1$Rain.y), factorize(dat1$Snow.y),
        factorize(dat1$Hail.y), factorize(dat1$Thunder.y), factorize(dat1$Tornado.y),factorize(dat1$season))


for (i in 1:n){
a = biglm.big.matrix (ArrDelay ~ Year + Month + DayofMonth + DayOfWeek + Distance + TEMP.y + DEWP.y + SLP.y + VISIB.y + WDSP.y + MXSPD.y + PRCP.y + SNDP.y + UniqueCarrier + Origin + Dest + Fog.y + Rain.y + Snow.y + Hail.y + Thunder.y + Tornado.y + season, data = dat1[-flds[[i]],] )
x=b[flds[[i]],]
c=x%*%summary(a)[[2]][,1]
